...
void handle_new_port (session_t *session) {
        ...
        /* Parameters to receive the packet from the transport layer */
	char *packet;
	int packet_len = 0;
	int error;

        /* Holders for tha packets */
	handshake_pkt *hspkt_t;
	data_pkt *dpkt_t;
	fin_pkt *fpkt_t;
		...
		switch (packet[0]) {
			case PKT_TYPE_HANDSHAKE:
				...
				log_mesg("Received HANDSHAKE packet Type : %d Session : %d No of packets %d  file %s\n  ",hspkt_t->type, hspkt_t->session_id, 
                                ...
                                if (session->session_id != hspkt_t->session_id) {
                                        printf("Session Mismatch.. Something Wrong\n");
                                        break;
                                }
			...
			case PKT_TYPE_DATA:
				...
				if (session->next_frame_number != dpkt_t->seq_num) {
                                        printf("Sequence Number Mismatch Expected %d Received %d\n", session->next_frame_number, dpkt_t->seq_num);
				...
				/* Subtract  header (11 bytes) from the data packet to consider only the data part */
				packet_len -= 11;
				...
				/* Okay we got the packet proper , write it to the file */
...
/* This thread will take care of the new port */
		...
		switch (packet[0]) {
			...
			case PKT_TYPE_HELLO_REQUEST : 
				...
				hello_pkt_resp.type = PKT_TYPE_HELLO_RESPONSE;
                                ...
                                /* Check if the session was created now */
                                if (!new_session) {
                                        /* Old session, extract the session id and return */
        				hello_pkt_resp.session_id = session_get_neighbor_session_addr(addr, hpkt_t->session_id, hpkt_t->host_name);
                                } else {
                                        /* New session. Return session id from the packet */
